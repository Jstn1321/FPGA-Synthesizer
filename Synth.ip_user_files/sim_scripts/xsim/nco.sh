...
#
# Script generated by Vivado on Thu Jun 11 16:43:43 -0700 2026
...
#
# Usage        : nco.sh
#                nco.sh [-lib_map_path] [-step] [-keep_index] [-noclean_files]*
#                nco.sh [-rerun]
#                nco.sh [-reset_run]
#                nco.sh [-clean]
#                nco.sh [-reset_log]
...
# script info
echo -e "nco.sh - Script generated by export_simulation (Vivado v2025.2 (64-bit)-id)\n"
...
run()
{
  ...
  if [[ ($b_step == 1) ]]; then
    case $step in
      ...
      ;;
      * )
        echo -e "ERROR: Invalid or missing step '$step' (type \"./nco.sh -help\" for more information)\n"
        exit 1
      esac
  else
    init_lib
...
elaborate()
{
  xelab --incr --debug typical --relax --mt 2  -L xil_defaultlib -L xpm -L uvm -L unisims_ver -L unimacro_ver -L secureip --snapshot nco xil_defaultlib.nco xil_defaultlib.glbl -log elaborate.log
}

# RUN_STEP: <simulate>
simulate()
{
  xsim nco -key {Behavioral:sim_1:Functional:nco} -tclbatch cmd.tcl -log simulate.log
}
...
reset_run()
{
  files_to_remove=(xelab.pb xsim.jou xvhdl.log xvlog.log compile.log elaborate.log simulate.log xelab.log xsim.log run.log xvhdl.pb xvlog.pb nco.wdb xsim.dir libdpi.so)
...
check_arg_value()
{
  ...
  if [[ ($1 == "-lib_map_path") && ($2 == "") ]];then
    echo -e "ERROR: Simulation library directory path not specified (type \"./nco.sh -help\" for more information)\n"
    exit 1
  fi
}
...
usage()
{
  msg="Usage: nco.sh [-help]\n\
Usage: nco.sh [-step]\n\
Usage: nco.sh [-lib_map_path]\n\
Usage: nco.sh [-rerun]\n\
Usage: nco.sh [-reset_run]\n\
Usage: nco.sh [-clean]\n\
Usage: nco.sh [-reset_log]\n\
Usage: nco.sh [-keep_index]\n\
Usage: nco.sh [-noclean_files]\n\n\
